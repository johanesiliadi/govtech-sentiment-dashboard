...
if not df.empty:
    ...
    fig = px.bar(sent_count,x="sentiment",y="count",title="Sentiment Distribution",
                 color="sentiment",
    ...
    st.dataframe(df_display[["date","employee","department","message","sentiment","topic"]],
...
# ---------- ADAPTIVE QUESTIONNAIRE GENERATOR ----------
st.subheader("🧩 AI-Generated Next Questionnaire")
if client is None:
    st.info("Set OPENAI_API_KEY in Streamlit secrets to enable adaptive question generation.")
else:
    if st.button("Generate next questionnaire"):
        ...
        Based on the latest sentiment analysis results:
        {summary_text}

        Generate 5 short, open-ended questions for the next survey.
        - Focus on topics where sentiment is most negative or frustrated.
        ...
        try:
            ...
            st.markdown("### Suggested Questions for Next Survey:")
